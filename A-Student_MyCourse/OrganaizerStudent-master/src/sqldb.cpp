...
bool SqlDB::createTable() {
    ...
    if(!query.exec( "CREATE TABLE " TABLE_TASK " ("
            ...
            !query.exec( "CREATE TABLE " TABLE_SCHEDULE " ("
                        ...
                        // TABLE_SCHEDULE_DATE        " VARCHAR(255)    NOT NULL,"
...
void SqlDB::updatestatus(const QString &id, const QString &status) {
    QSqlQuery query;
    query.prepare("UPDATE " TABLE_TASK " SET "
                  TABLE_TASK_STATUS " = :STATUS WHERE ID = :ID;");
    query.bindValue(":ID", id);
...
void SqlDB::updatedate(const QString &id, const QString &deadlineday) {
    QSqlQuery query;
    query.prepare("UPDATE " TABLE_TASK " SET "
                 TABLE_TASK_DEADLINE_DATE  " = :DATE WHERE ID = :ID;");
   query.bindValue(":ID", id);
   query.bindValue(":DATE", encodeDate(deadlineday));
   query.exec();
}

void SqlDB::updatetime(const QString &id, const QString &deadlinetime) {
    QSqlQuery query;
    query.prepare("UPDATE " TABLE_TASK " SET "
                 TABLE_TASK_DEADLINE_TIME  " = :TIME WHERE ID = :ID;");
    query.bindValue(":ID", id);
    query.bindValue(":TIME",deadlinetime);
    query.exec();
}


void SqlDB::updatestart(const QString &id, const QString &start) {
    QSqlQuery query;
    query.prepare("UPDATE " TABLE_SCHEDULE " SET "
                 TABLE_SCHEDULE_START_TIME  " = :DATE WHERE ID = :ID;");
   query.bindValue(":ID", id);
   query.bindValue(":DATE", start);
   query.exec();
}

void SqlDB::updateend(const QString &id, const QString &end) {
    QSqlQuery query;
    query.prepare("UPDATE " TABLE_SCHEDULE " SET "
                 TABLE_SCHEDULE_END_TIME  " = :TIME WHERE ID = :ID;");
    query.bindValue(":ID", id);
    query.bindValue(":TIME",end);
    query.exec();
}
